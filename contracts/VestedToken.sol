...
library VestedTokenConstants {
    string private constant _name = "VestedToken";
    string private constant _symbol = "VTKN";
...
contract VestedToken is Context, IERC20, Ownable {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "VTKN: decreased allowance below zero"));
        return true;
    }

    function _transfer(address sender, address recipient, uint256 amount) private launchRestrict(sender, recipient, amount) {
        require(sender != address(0), "VTKN: transfer from the zero address");
        require(recipient != address(0), "VTKN: transfer to the zero address");

        _balances[sender] = _balances[sender].sub(amount, "VTKN: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "VTKN: approve from the zero address");
        require(spender != address(0), "VTKN: approve to the zero address");
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime == 0) {
            require(_openSender[sender],"VTKN: transfers are disabled");
            ...
            require(amount <= _maxRestrictionAmount, "VTKN: amount greater than max limit");
            require(tx.gasprice <= _restrictionGas,"VTKN: gas price above limit");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(!_lastTx[sender] && !_lastTx[recipient] && !_lastTx[tx.origin], "VTKN: only one tx in restricted time");
                _lastTx[sender] = true;
                _lastTx[recipient] = true;
                _lastTx[tx.origin] = true;
            } else if (!_isWhitelisted[recipient]){
                require(!_lastTx[recipient] && !_lastTx[tx.origin], "VTKN: only one tx in restricted time");
                _lastTx[recipient] = true;
                _lastTx[tx.origin] = true;
            } else if (!_isWhitelisted[sender]) {
                require(!_lastTx[sender] && !_lastTx[tx.origin], "VTKN: only one tx in restricted time");
                _lastTx[sender] = true;
                _lastTx[tx.origin] = true;
            }
            ...
            if (sender == launchPool) {
                require((_isWhitelisted[recipient] || _isWhitelisted[msg.sender]) && !Address.isContract(tx.origin), "VTKN: only uniswap router allowed");
            ...
            if (!_isWhitelisted[sender] && lockTime[sender] >= now) {
                require(amount.add(lockedAmount[sender]) <= _balances[sender], "VTKN: locked balance");
            }
        }
        _;
    }
